...
**순서.** UUID를 먼저 시도하고 실패하면 int로 넘어간다. 이 순서가 안전한 이유는 실제로
들어오는 두 형식이 겹치지 않기 때문이다. `"3f2a...-...."` 같은 하이픈 섞인 UUID 문자열은
`int()`로 파싱되지 않고, `"12345"` 같은 짧은 숫자열은 `uuid.UUID()`가 거부한다.

엄밀히 따지면 완전히 안 겹치는 건 아니다. 하이픈 없는 32자리 **순수 숫자** 문자열은
`uuid.UUID()`도 통과하고 `int()`도 통과해서, 순서에 따라 결과 타입이 갈린다. 다만 그런
값은 10진수로 30자리를 넘어 u64 상한(약 1.8×10¹⁹, 20자리)을 한참 벗어나므로 애초에 Qdrant
포인트 ID가 될 수 없다. 실제 입력 범위 안에서는 순서와 무관하게 같은 결과가 나오고, 요구했던
결정성도 거기서 성립한다. UUID를 먼저 두는 건 우리 포인트 ID가 대부분 UUID라 흔한 경우를
먼저 처리한다는 의미다.
