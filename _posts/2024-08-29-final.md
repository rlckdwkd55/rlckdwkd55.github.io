...
- 즉, 한 번 할당하면 다시 할당할 수 없다. 참조형이라면 참조 대상이 고정될 뿐, 그 객체의 내부 값까지 잠기는 것은 아니다.
- `static final` 상수는 이름을 모두 대문자로 쓰는 것이 기본 코딩 스타일이다. 아래 blank final처럼 인스턴스마다 값이 달라지는 final 필드는 일반 변수와 같이 소문자로 쓴다.
