...
![](/assets/img/posts/language/java/stack/stack-structure.jpg)
...
- 스택이 비어있을 때 사용하면 EmptyStackException 예외 발생
...
for (int i = 1 ; i <= 5 ; i++){  
    tmp = stk.peek();
    stk.push(tmp);          // Queue 예제의 offer(tmp)에 대응 — 크기를 유지한다
    System.out.println("stk.pop = " + stk.pop()); // 마지막 값 삭제 및 출력
}  
```
