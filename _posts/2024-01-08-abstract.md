...
- 추상 메서드가 하나라도 있으면 그 클래스는 반드시 추상 클래스여야 한다. (반대는 성립하지 않아, 추상 메서드가 없어도 abstract를 붙일 수 있다.)
...
추상 메서드를 하나 이상 갖는 경우가 대부분이지만 하나도 없어도 선언할 수 있고, 공통점은 직접 인스턴스화할 수 없다는 것이다.
